...
def updateThisDictionary(d):
    new = {}

    for item in d:
        toPress = buttonPresser(arrowGrid,item)
        w = [x + "A" for x in toPress.split("A")][:-1]
        for x in w:
            if x in new: new[x] += d[item]
            else: new[x] = d[item]
...
for keycode in keycodes:
    # get initial button sequences
    initial = buttonPresser(numGrid,keycode)
    numpad = buttonPresser(arrowGrid,initial)
    # from here, 'numpad' is typed on the arrowGrid, so we can use the dictionary technique

    # make the initial dictionary (this is improper, but will be fixed by the updater)
    d = {numpad:1}
    
    for x in range(24):
        d = updateThisDictionary(d)
    
    total += length(d) * int(keycode[:-1])
