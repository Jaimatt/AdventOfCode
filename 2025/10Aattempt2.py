# My first answer worked, but I can do it better.

# Try doing a breadth first search (kind of), not a DFS.
...
def pressButtons(pressed,machine,pathLength):
    goal = machines[machine]["goal"]
    buttons = machines[machine]["buttons"]

    # immediate end if we're taking longer than the max path length
    if len(pressed) >= pathLength:
    ...
    for x in range(len(buttons)):
        ...
        path = pressButtons(pressed+[x],machine,pathLength)
