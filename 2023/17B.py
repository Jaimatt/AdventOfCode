...
def visit(x):
    ...
    if steps < 10 and row+dr in range(rows) and col+dc in range(cols):
    ...
    dr, dc = cart(nd)
    nr, nc = row + dr, col + dc
    if steps > 3 and 0 <= nr < rows and 0 <= nc < cols:
...
# overtime, at 4000ms. Good enough, given I'm sick of this challenge